...
Router::prefix('teacher', function (RouteBuilder $routes) {
    // Register scoped middleware for in scopes.
    $routes->registerMiddleware('csrf', new CsrfProtectionMiddleware([
        'httpOnly' => true
    ]));

    $routes->connect('/', ['controller' => 'Teachers', 'action' => 'view']);
...
Router::scope('/', function (RouteBuilder $routes) {
    ...
    $routes->connect('/register', ['controller' => 'Students', 'action' => 'add']);
    $routes->connect('/login', ['controller' => 'Students', 'action' => 'login']);
    $routes->connect('/logout', ['controller' => 'Students', 'action' => 'logout']);
    $routes->connect('/code', ['controller' => 'Students', 'action' => 'code']);
    $routes->connect('/profile/*', ['controller' => 'Students', 'action' => 'view']);

    $routes->connect('/branches', ['controller' => 'Branches', 'action' => 'index']);
    ...
    $routes->connect('/instructors', ['controller' => 'Teachers', 'action' => 'index']);
    $routes->connect('/instructor/*', ['controller' => 'Teachers', 'action' => 'view']);

    $routes->connect('/events', ['controller' => 'Events', 'action' => 'index']);
    $routes->connect('/event/*', ['controller' => 'Events', 'action' => 'view']);

    $routes->connect('/term/*', ['controller' => 'Terms', 'action' => 'view']);
